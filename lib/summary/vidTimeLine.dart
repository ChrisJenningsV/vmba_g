...
class VidTimelineTile extends StatelessWidget {
  const VidTimelineTile({
    ...
    this.height = 100,
    LineStyle afterLineStyle = const LineStyle(color: Colors.black,),
...
class _TimelinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (paintIndicator) {
      ...
      double indicatorCenterPoint =
          position.objectSpace.start + indicatorStartGap + indicatorRadius;

      if( isFirst) {
        indicatorCenterPoint = position.objectSpace.start; // + indicatorRadius;
      }
  ...
  void _drawSingleLine(
    ...
    if (!isLast) {
      Offset beginBottomLine = axis == TimelineAxis.vertical
          ? Offset(
        centerAxis,
        paintIndicator || !drawGap
            ? position.objectSpace.center
...
*/
  ...
  void _drawAfterLine(Canvas canvas, double centerAxis, AxisPosition position) {
    ...
    if( isFirst) {
      beginBottomLine = Offset(centerAxis, 20 ); // position.secondSpace.start);
